...
#define S_FUNCTION_NAME InternetClient
...
const XXInteger xx_number_of_inputs = 46; //modify here 6*1+1*(NP=40)
const XXInteger xx_number_of_outputs = 44;//modify server 3+1*1+1*1*(NP=40)
...
XXDouble xx_V[92];		/* variables */ //modify here //modify server xx_number_of_outputs+xx_number_of_inputs+2
XXMatrix xx_M[2];		/* matrices */
XXDouble xx_U[44];		/* unnamed */ //modify server xx_number_of_outputs
...
static void mdlInitializeConditions (double *x0, SimStruct *S)
{
	...
	xx_M[0].mat = &xx_V[50];		/* in */  //modify here xx_number_of_inputs+4
	xx_M[0].rows = 1;
	xx_M[0].columns = 47;   //modify here  xx_number_of_inputs+1
	xx_M[1].mat = &xx_U[0];		/* xx_U1 */
	xx_M[1].rows = 1;
	xx_M[1].columns = 44; //modify server xx_number_of_outputs
    ...
    //xx_M[0].mat[3:46] = zeros(44,1);
    xx_M[0].mat[4] = 200.0;  /* this one is for T_TB. Do not initialize to 0
                              */
...
static void mdlDerivatives (double *dx, double *x, double *u, SimStruct *S, int tid)
{
    ...
    // xx_V[3] = u[2];		/* throttle */
    iloop = 1;
    for (iloop = 1; iloop < 45; iloop++) {   //modify here iloop<xx_number_of_inputs-2+1
    ...
    iloop = 1;
    for (iloop = 1; iloop < 45; iloop++) {   //modify here iloop<xx_number_of_inputs-2+1
        xx_M[0].mat[iloop+2] = xx_V[iloop+2];}
...
static void mdlOutputs (double *y, double *x, double *u, SimStruct *S, int tid)
{
	...
	/* [engine.e, actualTime, dropRate, rtt] = dll (dll_name, function_name, in); */
	udpClient_CommunicateThroughUDP (xx_M[0].mat, 48, xx_M[1].mat, 44, xx_major);
    //modify here xx_M[0].mat, xx_number_of_inputs+2
    //modify server xx_M[1].mat, xx_number_of_outputs
	/* [engine.e, actualTime, dropRate, rtt] = dll (dll_name, function_name, in); */
// 	xx_V[1] = xx_M[1].mat[0];
// 	xx_V[5] = xx_M[1].mat[1];
// 	xx_V[3] = xx_M[1].mat[2];
...
//     
    ...
    iloop = 1;
    for (iloop = 1; iloop < 41; iloop++) {  //modify server iloop<NP+1
