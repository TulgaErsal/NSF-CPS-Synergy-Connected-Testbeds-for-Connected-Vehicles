...
int udpClient_CommunicateThroughUDP (double *inarr, int inputs, double *outarr, int outputs, int major)
{
    ...
    if (simStep++ % noOfFunctionCalls == 0) //if this is a major calculation
    {
        ...
        else
        {
            ...
            ufp = fopen("packetsServer_BBK5.txt","w");
            fprintf(ufp,"%d\t%f\t%f\t%f\t%f\t%f\t%f\n", fromServer.packetNumber, timeNow+((double)G.LIstartTime.QuadPart) / ((double)(G.LIticksPerSecond.QuadPart)), fromClient.sendSimTime, fromServer.sendSimTime, fromServer.engineTorque, fromServer.v_opt_array[0], fromServer.v_opt_array[0]);
            fclose(ufp);
        }
    }
