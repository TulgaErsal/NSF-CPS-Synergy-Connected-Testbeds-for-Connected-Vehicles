...
const char * serverIPAddress = "127.0.0.1";// This PC
...
typedef struct
{
    ...
    float  v_lead ;
    float  v_ego ;
    float  v_lead_pre[40] ; //modify here NP
} LH_SharedData;
...
typedef struct
{
    double simTime;
    double engineTorque;
    double v_opt_array[40]; //modify server
...
double abstimeNow;   // actual system time
...
int udpServer_Initialize ()
{
    ...
    ofp = fopen("logSolver.txt","w");
    tfp = fopen("packetsClient_Solver.txt","w");
    ufp = fopen("packetsServer_Solver.txt","w");
...
//         fprintf(ofp,"LItNow.QuadPart is %f .\n",LItNow.QuadPart);
...
int udpServer_CommunicateThroughUDP (double *inarr, int inputs, double *outarr, int outputs, int major)
{
    ...
    if (myMajor)
    {
        ...
        actualTime = (double)(tickCPU.QuadPart-initialTick.QuadPart)/(double)G.LIticksPerSecond.QuadPart;
        // Chunan change this:
        abstimeNow = (double)(tickCPU.QuadPart)/(double)G.LIticksPerSecond.QuadPart;
        ...
        if (!G.replySent)
        {
            ...
            fprintf (tfp,"%i\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n",fromClient.packetNumber, actualTime, abstimeNow, myIns.simTime, fromClient.sendSimTime, fromClient.throttle, fromClient.engineSpeed, fromClient.dropRate, fromClient.rtt);
            //Record Server packet
            fprintf (ufp,"%d\t%f\t%f\t%f\t%f\t%f\t%f\n", fromServer.packetNumber, actualTime, abstimeNow, fromServer.sendSimTime, fromServer.engineTorque, fromServer.v_opt_array[0], fromServer.v_opt_array[1]);
